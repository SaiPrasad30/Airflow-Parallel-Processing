...
def save_joined_data_s3(task_instance):
    ...
    df.to_csv(f"s3://airflow-testing-parallel-processing/transformed_data/{dt_string}.csv", index=False)
...
with DAG('weather_dag_parallel_processing',
        ...
        with TaskGroup(group_id = 'group_a', tooltip= "Extract_from_S3_and_weatherapi") as group_A:
            ...
            upload_data_from_s3_to_table = PostgresOperator(
                task_id = "upload_data_from_s3_to_table",
                postgres_conn_id = "postgres_conn",
                sql = 
                    '''
                    ...
                    'airflow-testing-parallel-processing',
                    'us_city.csv', 
                    'ap-south-1');
                    ''' 
            )
            ...
            create_table_1 >> truncate_table >> upload_data_from_s3_to_table
...
start_pipeline >> group_A >> join_data >> load_joined_data
